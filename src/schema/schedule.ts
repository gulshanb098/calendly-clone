      ...
      z.object({
        ...
        startTime: z
          ...
          .regex(
            ...
            "Time must be in this format (HH:MM)"
          ),
        endTime: z
          .string()
          .regex(
    ...
    .superRefine((availabilities, ctx) => {
      availabilities.forEach((availability, index) => {
        ...
        if (overlaps) {
          ctx.addIssue({
            code: "custom",
            message: `Availability on ${availability.dayOfWeek} overlaps with another availability.`,
            path: [index],
          });
        }
        ...
        if (
          ...
          ctx.addIssue({
            code: "custom",
            message: `End time must be after start time on ${availability.dayOfWeek}.`,
            path: [index],
          });
        }
      });
